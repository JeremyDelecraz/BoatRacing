...
function Sprite(ctx, img, x, y, frame, anim, rate) {
   ...
   this.hurt = false; //Définni si le bateau s'est fait touché ou non.
   this.shield = 0; //Bouclier pour quand deux bateaux se rentrent dedans.
   ...
   this.Show = function() { //Fonction pour afficher le sprite.
      ...
      //On baisse le numéro du bouclier.
      this.shield--;
   ...
   this.Move = function() { //Fonction qui va faire bouger le vaisseau selon les touches du clavier.
      //On change la position en x et en y en ajoutant la vitesse multipliée par l'angle à la position d'avant.
   ...
   this.SetAnim = function(num) { //Fonction qui va gérer quelle partie de l'image va appraître.
      //On prend le numéro muliplié par la hauteur et moins la hauteur pour avoir par exemple la position 0 quand on donne 1 en paramètre.
   ...
   this.Colision = function(opponentBoat) { //Fonction qui va gérer la collision, prend en paramètre un bateau adversaire.
      //On fait pythagore pour savoir si le rayon de l'un va dans le rayon de l'autre.
   ...
   {
      //Note : 96 est la taille de l'image en largeur.
      if (this.sx === 96)
      {
         return true;
      }
